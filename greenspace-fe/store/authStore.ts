  ...
  actions: {
    async login(payload: { identifier: string; password: string; role: string }) {
      if (payload.role === "USER") {
        try {
          const response: Response<User> = await $fetch(`${this.$nuxt.$config.public.baseUrl}/auth/login/user`, { method: "POST", body: payload });
        ...
        try {
          const response: Response<Officer> = await $fetch(`${this.$nuxt.$config.public.baseUrl}/auth/login/officer`, { method: "POST", body: payload });
          this.token = response.token;
