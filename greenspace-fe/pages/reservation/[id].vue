...
const fetchDetailParkingSpot = async () => {
  try {
    const response: Response<ParkingSpot> = await $fetch(`${import.meta.env.VITE_BASE_ENDPOINT}/parking/spot/${route.params.id}`, { method: "GET", withCredentials: true, credentials: "include" });
...
const submitReservation = async () => {
  ...
  try {
    const response: Response<Reservation> = await $fetch(`${import.meta.env.VITE_BASE_ENDPOINT}/reserve/reservation`, { method: "POST", body: payload, withCredentials: true, credentials: "include" });
